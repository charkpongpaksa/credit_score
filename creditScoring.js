 ...
 * Logistic Regression-based model with weighted feature scoring.
 * Scores range 300–850 (similar to FICO scale).
 *
 * Feature weights are calibrated for Thai retail credit market.
 * Ready to swap with a real API call to your ML backend.
 */
...
const WEIGHTS = {
  ...
  // Employment stability (10% of score)
  employment: {
    government:       0.10,
...
function getDSRCategory(monthlyIncome, monthlyDebt) {
  if (!monthlyIncome || monthlyIncome === 0) return 'over50'
...
/**
 * @param {Object} formData - collected from the 4-step form
 * @returns {Object} { score, riskLevel, riskLabel, factors, recommendation, breakdown }
 */
export function calculateCreditScore(formData) {
  ...
  // totalWeight ranges from roughly -1.0 to +1.0
  // Map to FICO-like 300–850
  const minW = -1.0
  const maxW = 1.0
  const normalized = (totalWeight - minW) / (maxW - minW)
  ...
  const factors = Object.entries(breakdown)
    .map(([k, v]) => ({ ...v, id: k }))
    .sort((a, b) => a.weight - b.weight) // worst first
    .slice(0, 3)
    .map((f) => ({
      id: f.id,
      label: f.label,
